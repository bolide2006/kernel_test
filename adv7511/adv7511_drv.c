                   ...
                   bool rgb, bool hdmi_mode)
{
    ...
    config.avi_infoframe.scan_mode = HDMI_SCAN_MODE_UNDERSCAN;
            printk("%s is called with rgb %d powered %d \n",__func__,rgb,adv7511->powered);
    if (rgb) {
                 ...
                 const struct drm_display_mode *adj_mode)
{
    ...
    unsigned int vsync_polarity = 0;
        printk("%s h %d v %d\n",__func__,adj_mode->crtc_hdisplay,adj_mode->crtc_vdisplay);
...
static void adv7511_bridge_enable(struct drm_bridge *bridge)
{
    ...
    //FIXME_fzhou   only for sbs dualink two HDMI display

    if(adv->i2c_main->addr == 57 && advBakcup[1])
    {
        //adv7511_power_on(advBakcup[1]);
    }
}
                    ...
                    const struct drm_display_mode *adj_mode)
{
    ...
    if(adv->i2c_main->addr == 57 && advBakcup[1])
    {
        //adv7511_mode_set(advBakcup[1], mode, adj_mode);
    }
...
static void setDefaultRegSetting(struct adv7511 *adv7511)
{
    int i = 0;
    int j = 0;
ADV7511_REG_T  regSetting[]={
    {0x41,0x00},
    {0x98,0x03},
    {0x9A,0xE0},
    {0x9C,0x30},
    ...
    i = sizeof(regSetting)/sizeof(ADV7511_REG_T);
    printk("Adv7511 def setting cnt = %d\n",i);

    for(j = 0; j < i;j++)
    {
        printk("Adv7511 def setting addr %x =  val %x\n",regSetting[j].addr,regSetting[j].value);

        regmap_write(adv7511->regmap, regSetting[j].addr,
                 regSetting[j].value);
    }
...
static int adv7511_encoder_init(struct device *dev, struct drm_device *drm)
{
...
static int adv7511_bind(struct device *dev, struct device *master, void *data)
{
    ...
    return adv7511_encoder_init(dev, drm);
...
static int adv7511_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
    ...
    if (!dev->of_node)
        return -EINVAL;
         ...
         if(testCnt == 2)
         {
            cecAddr = ADV7511_CEC_I2C_ADDR_DEFAULT;
            edidAddr = ADV7511_EDID_I2C_ADDR_DEFAULT;
            packAddr = ADV7511_PACKET_I2C_ADDR_DEFAULT;
         }
         else
         {
             cecAddr = ADV7511_CEC_I2C_ADDR_DEFAULT+10;
             edidAddr = ADV7511_EDID_I2C_ADDR_DEFAULT+10;
             packAddr = ADV7511_PACKET_I2C_ADDR_DEFAULT+10;
         }
    ...
    adv7511->status = connector_status_disconnected;
         ...
         if(i2c->addr == 57)advBakcup[0] = adv7511;
         else advBakcup[1] = adv7511;
    if (dev->of_node)
    ...
    adv7511_audio_init(dev, adv7511);
        ...
        setDefaultRegSetting(adv7511);
#endif
        printk("adv7511 probed %d \n",ret);
    return 0;
